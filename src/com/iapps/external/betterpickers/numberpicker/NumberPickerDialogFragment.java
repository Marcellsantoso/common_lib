package com.iapps.external.betterpickers.numberpicker;
...
public class NumberPickerDialogFragment
	...
	private static final String					REFERENCE_KEY				= "PinPickerDialogFragment_ReferenceKey";
	private static final String					THEME_RES_ID_KEY			= "PinPickerDialogFragment_ThemeResIdKey";
	private static final String					MIN_NUMBER_KEY				= "PinPickerDialogFragment_MinNumberKey";
	private static final String					MAX_NUMBER_KEY				= "PinPickerDialogFragment_MaxNumberKey";
	private static final String					MAX_LENGTH					= "PinPickerDialogFragment_MaxLength";
	private static final String					MIN_LENGTH					= "PinPickerDialogFragment_MinLength";
	private static final String					PLUS_MINUS_VISIBILITY_KEY	=
																					"PinPickerDialogFragment_PlusMinusVisibilityKey";
	private static final String					DECIMAL_VISIBILITY_KEY		=
																					"PinPickerDialogFragment_DecimalVisibilityKey";
	private static final String					LABEL_TEXT_KEY				= "PinPickerDialogFragment_LabelTextKey";
	...
	public static NumberPickerDialogFragment newInstance(
			int reference, int themeResId, Integer minNumber,
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (args != null && args.containsKey(MAX_LENGTH)) {
			mMaxLength = args.getInt(MAX_LENGTH);
		}
		if (args != null && args.containsKey(MIN_LENGTH)) {
			mMinLength = args.getInt(MIN_LENGTH);
		}
		...
		mDividerColor = getResources().getColor(
				R.color.default_divider_color_dark);
		mDialogBackgroundResId = R.drawable.dialog_full_holo_dark;

		if (mTheme != -1) {
			TypedArray a = getActivity().getApplicationContext()
					.obtainStyledAttributes(mTheme,
							R.styleable.BetterPickersDialogFragment);

			mTextColor = a
					.getColorStateList(R.styleable.BetterPickersDialogFragment_bpTextColor);
			mButtonBackgroundResId = a.getResourceId(
					R.styleable.BetterPickersDialogFragment_bpButtonBackground,
					mButtonBackgroundResId);
			mDividerColor = a.getColor(
					R.styleable.BetterPickersDialogFragment_bpDividerColor,
					mDividerColor);
			mDialogBackgroundResId = a.getResourceId(
					R.styleable.BetterPickersDialogFragment_bpDialogBackground,
	...
	public View onCreateView(
		...
		mSet.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View view) {
				...
				if (mMinNumber != null && mMaxNumber != null
						&& (number < mMinNumber || number > mMaxNumber)) {
					String errorText = String.format(
							getString(R.string.min_max_error), mMinNumber,
							mMaxNumber);
					...
					String errorText = String.format(
							getString(R.string.min_error), mMinNumber);
					mPicker.getErrorView().setText(errorText);
					mPicker.getErrorView().show();
					return;
				}
					...
					String errorText = String.format(
							getString(R.string.max_error), mMaxNumber);
					mPicker.getErrorView().setText(errorText);
					mPicker.getErrorView().show();
					return;
				}
				for (NumberPickerDialogHandler handler : mPinPickerDialogHandlers) {
					handler.onDialogNumberSet(mReference, mPicker.getNumber(),
				...
				if (activity instanceof NumberPickerDialogHandler) {
					...
					act.onDialogNumberSet(mReference, mPicker.getNumber(),
							mPicker.getDecimal(), mPicker.getIsNegative(),
							number);
				}
	...
	/**
	 * Attach a Vector of handlers to be notified in addition to the Fragment's Activity and target
	 * Fragment.
	 * 
	 * @param handlers a Vector of handlers
	 */
	public void setNumberPickerDialogHandlers(
			Vector<NumberPickerDialogHandler> handlers) {
		mPinPickerDialogHandlers = handlers;
	}
}